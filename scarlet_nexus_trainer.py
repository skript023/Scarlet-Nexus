...
def main():
    ...
    with imgui.window(id="Primary Window", label="Scarlet Nexus Trainer", width=600, height=400, no_move=True, min_size=[250,250], no_collapse=True, on_close=lambda:sys.exit(0)):
        ...
        with imgui.tab_bar(label="Tabbar"):
            with imgui.tab(label="Player Option"):
                ...
                imgui.add_slider_int(label="Attack", callback=lambda:pm.write_int(player.getCharacterBase() + 0xAEC, imgui.get_value("attack")), id="attack", max_value=1000, width=200, default_value=pm.read_int(player.getCharacterBase() + 0xAEC))
                imgui.add_slider_int(label="Brain Dive Attack", callback=lambda:pm.write_int(player.getCharacterBase() + 0xAF0, imgui.get_value("brain_attack")), id="brain_attack", max_value=1000, width=200, default_value=pm.read_int(player.getCharacterBase() + 0xAF0))
                imgui.add_separator()
                imgui.add_text("Skill Changer")
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_1")), 1), id="slot_1", label="Slot 1", default_value=player.get_player_skill(1), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_2")), 2), id="slot_2", label="Slot 2", default_value=player.get_player_skill(2), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_3")), 3), id="slot_3", label="Slot 3", default_value=player.get_player_skill(3), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_4")), 4), id="slot_4", label="Slot 4", default_value=player.get_player_skill(4), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_5")), 5), id="slot_5", label="Slot 5", default_value=player.get_player_skill(5), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_6")), 6), id="slot_6", label="Slot 6", default_value=player.get_player_skill(6), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_7")), 7), id="slot_7", label="Slot 7", default_value=player.get_player_skill(7), width=200)
                imgui.add_combo(player.skill_list, callback=lambda:player.set_player_skill(player.skill_list.index(imgui.get_value("slot_8")), 8), id="slot_8", label="Slot 8", default_value=player.get_player_skill(8), width=200)
